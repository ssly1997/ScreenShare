...
- (void)socket:(GCDAsyncSocket *)sock didConnectToHost:(NSString *)host port:(uint16_t)port {
    self.connected = YES;
    if ([self.delegate respondsToSelector:@selector(clientSocket: status:)]) {
        dispatch_async(dispatch_get_main_queue(), ^{
            [self.delegate clientSocket:self status:(ConnectStatusConnected)];
        });
    }
...
- (void)socket:(GCDAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag {
    ...
    while (self.dataBuffer.length >= 4) {
        ...
        if (self.dataBuffer.length >= (dataLength+4)) {
            ...
            if ([self.delegate respondsToSelector:@selector(clientSocket: receiveData:)]) {
                [self.delegate clientSocket:self receiveData:realData];
            }
...
- (void)socketDidDisconnect:(GCDAsyncSocket *)sock withError:(nullable NSError *)err {
    ...
    if ([self.delegate respondsToSelector:@selector(clientSocket: status:)]) {
        dispatch_async(dispatch_get_main_queue(), ^{
             [self.delegate clientSocket:self status:(ConnectStatusDisconnected)];
        });
    }
    NSLog(@"%@", [NSThread currentThread]);
    [self stopHeartTimer];
